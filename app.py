...
try:
    ...
    st.error("CRITICAL: 'google-genai' library missing. Please run: pip install google-genai")
...
def generate_flashcards(api_key, content_input, difficulty, count_val, is_image=False):
    ...
    try:
        ...
        # Use gemini-1.5-flash which supports images and text
...
def get_card_style():
    # Force black text on white card, regardless of Streamlit theme
    return """
    <style>
    .flashcard {
                ...
                try:
                    with get_db_connection() as conn:
                        ...
                        if res:
                            ...
                            time.sleep(1)
                            st.session_state["menu_selection"] = "Library" 
                            st.rerun()
                except Exception as e:
                    st.error(f"Database Error: {e}")
            else:
                st.error("No cards generated.")
...
def section_study_mode():
    ...
    if idx < len(queue):
        ...
        # Card UI with Forced Colors
        with st.container():
            st.markdown(f"""
            <div class="flashcard">
                <div class="flashcard-tag">{card['tag']}</div>
...
def section_settings():
    ...
    st.info("API Key settings are available in the sidebar.")
...
def main():
    ...
    selection = st.sidebar.radio(
        ...
        index=["Study Mode", "Generator", "Library", "Settings"].index(st.session_state["menu_selection"])
    )
    
    if selection != st.session_state["menu_selection"]:
        st.session_state["menu_selection"] = selection
        st.rerun()
    
    if st.session_state["menu_selection"] == "Study Mode":
        section_study_mode()
    elif st.session_state["menu_selection"] == "Generator":
        section_generator()
    elif st.session_state["menu_selection"] == "Library":
        section_library()
