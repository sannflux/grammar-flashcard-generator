...
def get_piped_transcript(video_id):
    """Bypasses YouTube IP Bans by routing through Decentralized Piped APIs."""
    instances = [
    ...
    for instance in instances:
        try:
            ...
            if resp.status_code == 200:
                ...
                # Find English track (Prefer manual, fallback to auto)
                target_sub = next((s for s in subtitles if s.get('code') == 'en' and not s.get('autoGenerated')), None)
                if not target_sub:
                    target_sub = next((s for s in subtitles if s.get('code') == 'en'), subtitles[0])
                    
                sub_url = target_sub.get('url')
                if sub_url:
                    # Fetch raw WebVTT file
                    vtt_resp = requests.get(sub_url, timeout=10)
                    
                    # Clean the VTT formatting
                    clean_lines = []
                    for line in vtt_resp.text.split('\n'):
                        ...
                        if clean_line and clean_line not in clean_lines[-1:]:
                            clean_lines.append(clean_line)
                            
                    result_text = " ".join(clean_lines)
                    if result_text:
                        return result_text
        except Exception:
            continue
            
    raise ValueError("Decentralized proxy network could not retrieve transcripts.")
            ...
            if url:
                with st.spinner("Transcribing..."):
                    try:
                        ...
                        # Plan A: Local Library (Works on local machine, fails on IP-banned Cloud)
                        ...
                        # Plan B: Decentralized Piped Network (Immune to IP bans)
                        if not raw_text:
                            raw_text = get_piped_transcript(video_id)
